...
import type { AvailableSpaceValue, GridTemplateComponent, NodeId } from "../src/index.js";
...
test("generated fixture: leaf/leaf_with_content_and_padding_border", () => {
...
test("generated fixture: flex/gap_column_gap_determines_parent_width", () => {
  for (const boxSizing of [BoxSizing.BorderBox, BoxSizing.ContentBox]) {
    const taffy = TaffyTree.new();
...
test("generated fixture: block/block_margin_y_sibling_collapse_positive_and_negative", () => {
  for (const boxSizing of [BoxSizing.BorderBox, BoxSizing.ContentBox]) {
    const taffy = TaffyTree.new();
    const children = [
...
test("generated fixture: grid/grid_align_content_space_evenly", () => {
  for (const boxSizing of [BoxSizing.BorderBox, BoxSizing.ContentBox]) {
    const taffy = TaffyTree.new();
...
function testMeasureFunction(
  knownDimensions: Size,
  availableSpace: Size,
  _nodeId: NodeId<AhemTextContext>,
...
function measureAhemText(
  context: AhemTextContext,
  knownDimensions: Size,
  availableSpace: Size,
): Size {
  ...
  const inlineSize = Math.max(
    knownDimensions[inlineAxis] ??
      resolveAvailableInlineSize(inlineAvailable, minLineLength, maxLineLength),
    minLineLength * 10,
  );
