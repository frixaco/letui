...
import { NODE_KIND_ID, type NodeKind, type NodeKindNum, type NormalizedTextSpan } from "./types.ts";
...
function encodeTextSpanAttrFlags(span: NormalizedTextSpan): number {
  let flags = 0;
  if (span.bold) flags |= TEXT_SPAN_ATTR_BOLD;
  if (span.italic) flags |= TEXT_SPAN_ATTR_ITALIC;
  if (span.underline) flags |= TEXT_SPAN_ATTR_UNDERLINE;
  return flags;
}

function encodeTextSpanColorFlags(span: NormalizedTextSpan): number {
  let flags = 0;
  if (span.foreground !== undefined) flags |= TEXT_SPAN_COLOR_FOREGROUND;
  if (span.background !== undefined) flags |= TEXT_SPAN_COLOR_BACKGROUND;
  return flags;
}
